...
export function renderListStatus(status, err) {
    ...
    if (status === "loading") {
        ...
        el.textContent = "📭 Інцидентів ще немає.";
...
export function renderPagination(meta, currentPage, onPage) {
    ...
    if (start > 1)
        el.appendChild(makePageBtn("1", false, () => onPage(1)));
    if (start > 2)
    ...
    if (end < totalPages - 1)
        el.appendChild(makeEllipsis());
    if (end < totalPages)
        el.appendChild(makePageBtn(String(totalPages), false, () => onPage(totalPages)));
...
export function renderThreatStats(items) {
    const tbody = document.getElementById("threatStatsBody");
    tbody.innerHTML = "";
    if (!items?.length) {
        const tr = document.createElement("tr");
        const cell = document.createElement("td");
...
export function renderEndpointResult(value) {
    const el = document.getElementById("detailsResult");
    if (!el)
        return;
    el.textContent = JSON.stringify(value, null, 2);
}
export function renderStats(items, mode = "all") {
